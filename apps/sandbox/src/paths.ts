 ...
 * than forwarded. A proxy that forwards unknown paths is an open relay to
 * somebody's Supabase project, and what is reachable that way grows every time
 * Supabase ships a new service.
 */
 ...
 * `/rest`, classifying an unknown path as a known one.
 */
 ...
 *
 * Normalizing is not this function's job and must not be done here with string
 * manipulation. `new URL()` resolves `..` and `.` segments, so classifying
 * `url.pathname` is safe where classifying a raw request-target string is not:
 * a hand-rolled `startsWith` against `/storage/v1/../../rest/v1/users` would
 * answer "storage" for a request the origin treats as REST.
 *
 ...
 * that names `/pg/query` the moment anything downstream decodes it. The
 * allowlist cannot be the security boundary while the string it matches differs
 * from the string the origin routes on, and we do not get to assume the
 * Supabase gateway routes before decoding.
 *
 * So the path is decoded, re-resolved, and classified a second time. It is
 * refused only when decoding CHANGES the answer, which keeps a storage key
 * legitimately containing `%2F` classified as storage while refusing a
