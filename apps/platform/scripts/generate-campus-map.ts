...
const QUERY_BBOX = [33.944, -83.387, 33.9535, -83.374] as const;

/** Rendered frame, chosen to hold Brumby west, MLC/Tate east, Baxter north,
 * Oglethorpe Dining south. */
const S = 33.9462,
  N = 33.9525,
  W = -83.3832,
  E = -83.3738;
...
const buildings = BUILDING_NAMES.map((name) => {
  const b = elements.find((e) => e.tags?.building && e.tags.name === name);
...
// the DLW (x < 200 puts the last point at the building's southeast corner).
const uc = proj(way(UNIVERSITY_CT_WAY_ID));
const stopIdx = uc.findIndex((p) => p[0] < 200);
routePts.push(...uc.slice(0, stopIdx + 1));
// Start the line at Tate's west side rather than in the middle of Lumpkin.
const start: [number, number] = [routePts[0]![0] + 55, routePts[0]![1] - 6];
const route =
  "M" +
  [start.map(fmt), ...simplify(routePts)].map((p) => p.join(" ")).join("L");
const routeEnd = routePts[routePts.length - 1]!.map(fmt) as [number, number];
