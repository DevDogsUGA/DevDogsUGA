 ...
 * the page anybody came for. Everything below it — the calendar, the schedule
 * — is context for this one card.
 *
 ...
 * explicit `timeZone` — the pure equivalent, and what the homepage calendar
 * already uses.
 *
...
interface Props {
   ...
   *
   * A prop rather than a read, and shared rather than per-band, so the
   * marquee's "in 3 days" and the calendar's highlighted day cannot disagree
 ...
 * render for a card that shows one date is waste the prerender pays for.
 */
...
/**
 * The band's primary actions, at the size the footer of `EventsSection` uses.
 *
...
const CARD_CLS =
  "shadow-block-lg rounded-sm border-2 border-black bg-white p-6 shadow-black md:p-8";

export default function Marquee({ meeting, now, checkIn }: Props) {
  ...
  // `now` is baked at prerender while the card is served from cache, so it can
  ...
  return (
    <article className={CARD_CLS} data-animate="fade-up">
      {/* `min-w-0` on the text column, not just here: a flex child defaults to
          ...
          scroll. The date column is `shrink-0` for the opposite reason — it
          must not be squeezed by the headline beside it. */}
      <div className="flex flex-col gap-5 md:flex-row md:items-start md:gap-8">
        <DateBlock at={meeting.startsAt} />

        <div className="flex min-w-0 flex-1 flex-col gap-4">
          <div className="flex flex-col gap-2">
            <h2 className="font-display text-4xl font-extrabold text-balance break-words text-black md:text-5xl">
              {meeting.name}
            </h2>
          ...
          {/* One line, not the agenda: the full list — with judging times, team
              ...
              duplicating it here would give the marquee two jobs and make the
              detail link pointless. */}
          {agenda !== null && (
            <p className="line-clamp-2 text-sm text-mauve-600">
...
/**
 * Weekday over the day number — the only display-scale numerals on the page,
 * so the date is findable from across a room without competing with the
 * meeting's name for the same emphasis.
 *
 ...
 * already tomorrow in UTC, and the card would print the wrong number for every
...
function DateBlock({ at }: { at: Date }) {
  return (
    <time
      ...
      className="flex shrink-0 flex-col leading-none md:w-24 md:items-center"
    >
      <span className="font-display text-sm font-extrabold tracking-widest text-mauve-500 uppercase">
        {WEEKDAY_FMT.format(at)}
      </span>
      <span className="font-display text-6xl font-extrabold text-black tabular-nums md:text-7xl">
        {DAY_FMT.format(at)}
      </span>
    </time>
  );
 ...
 * empty box would read as something broken, and "no events" says nothing to
  ...
  return (
    <article className={CARD_CLS} data-animate="fade-up">
      <div className="flex flex-col gap-4">
        ...
        <h2 className="font-display text-4xl font-extrabold text-balance text-black md:text-5xl">
          Nothing on the calendar yet
        </h2>

        <p className="max-w-prose text-base/relaxed text-mauve-700">
          The fall schedule goes up in August, and once it does there is
          something every week: a workshop that teaches a feature, a week-long
          competition to build it, and an open build session on the Wednesday in
          between. Everything happens in DLW 124.
        </p>
