...
import { useVerification } from "./NavUserProvider";
...
export default function VerificationAlert() {
  ...
  return (
    <button
      type="button"
      onClick={openDialog}
      className="relative flex w-full flex-col gap-1.5 overflow-hidden rounded-sm border border-rose-900/60 bg-rose-950/40 px-2.5 py-2 text-left transition-colors hover:border-rose-800/80"
    >
      ...
      <span className="relative flex items-center gap-1.5 text-xs font-semibold text-rose-200">
        <ShieldWarningIcon weight="bold" className="shrink-0" />
        Finish Account Setup
      </span>
      <ProgressBar value={completed} max={total} fillColor="amber" />
