 ...
 * same question the project tiles ask, so the row carries no glyph of its own
      ...
      <div className="relative flex cursor-pointer items-center justify-center gap-2.5 rounded-sm border border-black bg-white px-4 py-2 text-mauve-950 transition-colors hover:bg-mauve-100">
        {entry.favicon && (
          <Image
            ...
            // Rendered at 16px from a 64px source, so no pipeline pass is
            // needed. That is also why there is no `sizes`: `unoptimized` emits
            // a bare src and drops srcset and sizes both, so one here would be
            // dead weight. The static import still carries its intrinsic
            // 64×64, so the box is reserved before the file lands.
            unoptimized
          />
        )}
