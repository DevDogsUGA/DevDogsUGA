 ...
 * when it closes. Opened from the page behind it, there is a history entry of
 * ours behind the dialog and `router.back()` removes it; landed on directly — a
 ...
 * time anyway since each one is a route. Keying it per dialog would encode a
 * distinction that does not exist, and would still answer the same for both:
 * after a cold load every key is false, after any in-app navigation the one
 * that matters is true.
 *
 ...
 * the cases the browser handles itself (a middle click, a new tab) — those
 * really are fresh loads.
 */
let inApp = false;
