 ...
 * `sb-<ref>-auth-token-code-verifier` cookie and redirects to Google, so it is
 * state-changing despite being a GET. `<Link>` prefetches it like any other
 * route, which meant every page view silently began two handshakes before the
 * visitor clicked anything.
 *
 ...
 * nothing: the redirect target is cross-origin, so the router could never have
 * consumed the response, always fell back to a hard navigation, and started the
 * handshake a second time anyway.
 */
