import { pgSchema, pgTable, uuid, varchar, text, integer, pgEnum, date, timestamp, doublePrecision, boolean, jsonb, customType, uniqueIndex, index, foreignKey, primaryKey, unique, check, pgPolicy } from "drizzle-orm/pg-core"
...
export const reportStatusInPlatform = platform.enum("reportStatus", ["unverified", "pending", "resolved", "dismissed"])
...
export const contentReportsInPlatform = platform.table.withRLS("contentReports", {
	...
	foreignKey({
		...
		foreignColumns: [reportContentTypesInPlatform.clientId, reportContentTypesInPlatform.id],
		name: "contentReports_clientId_contentTypeId_fkey"
	}).onDelete("restrict"),
	foreignKey({
		columns: [table.clientId, table.reasonId],
		foreignColumns: [reportReasonsInPlatform.clientId, reportReasonsInPlatform.id],
...
export const docsBranchesInPlatform = platform.table.withRLS("docsBranches", {
	id: uuid().defaultRandom().primaryKey(),
	repoId: uuid().notNull().references(() => docsReposInPlatform.id, { onDelete: "cascade" } ),
	name: text().notNull(),
...
export const docsPagesInPlatform = platform.table.withRLS("docsPages", {
	id: uuid().defaultRandom().primaryKey(),
	branchId: uuid().notNull().references(() => docsBranchesInPlatform.id, { onDelete: "cascade" } ),
	path: text().notNull(),
...
export const docsReposInPlatform = platform.table.withRLS("docsRepos", {
	id: uuid().defaultRandom().primaryKey(),
	slug: text().notNull(),
	name: text().notNull(),
	description: text(),
...
export const moderatorRolesInPlatform = platform.table.withRLS("moderatorRoles", {
	id: uuid().defaultRandom().primaryKey(),
...
export const reportContentTypesInPlatform = platform.table.withRLS("reportContentTypes", {
	id: uuid().defaultRandom().primaryKey(),
	clientId: uuid().notNull().references(() => oauthClients.id, { onDelete: "cascade" } ),
...
export const reportCorroborationsInPlatform = platform.table.withRLS("reportCorroborations", {
	id: uuid().defaultRandom().primaryKey(),
	reportId: uuid().notNull().references(() => contentReportsInPlatform.id, { onDelete: "cascade" } ),
	reporterUserId: uuid().notNull().references(() => users.id, { onDelete: "cascade" } ),
	description: varchar({ length: 1000 }),
	createdAt: timestamp().default(sql`now()`).notNull(),
...
export const siteFeedbackInPlatform = platform.table.withRLS("siteFeedback", {
	id: uuid().defaultRandom().primaryKey(),
	userId: uuid().notNull().references(() => users.id, { onDelete: "cascade" } ),
	...
	foreignKey({
		...
		foreignColumns: [feedbackTopicsInPlatform.clientId, feedbackTopicsInPlatform.id],
