...
export const objectsInStorage = storage.table.withRLS(
  ...
  (table) => [
    ...
    pgPolicy("avatar_delete_policy", {
      for: "delete",
      to: ["authenticated"],
      using: sql`((bucket_id = 'avatars'::text) AND (name = (auth.uid())::text))`,
    }),

    pgPolicy("avatar_insert_policy", {
      for: "insert",
      to: ["authenticated"],
      withCheck: sql`((bucket_id = 'avatars'::text) AND (name = (auth.uid())::text) AND (path_tokens = ARRAY[(auth.uid())::text]))`,
    }),

    pgPolicy("avatar_update_policy", {
      for: "update",
      to: ["authenticated"],
      using: sql`((bucket_id = 'avatars'::text) AND (name = (auth.uid())::text) AND (path_tokens = ARRAY[(auth.uid())::text]))`,
      withCheck: sql`((bucket_id = 'avatars'::text) AND (name = (auth.uid())::text) AND (path_tokens = ARRAY[(auth.uid())::text]))`,
    }),
  ],
);
