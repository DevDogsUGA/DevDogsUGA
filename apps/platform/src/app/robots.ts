 ...
 * /robots.txt
 *
 * The disallow list is the complement of what `sitemap.ts` names, worked out
 * against the route tree rather than from memory. Every entry is a real first
 * segment under `app/(site)` or `app/(api)`, and every public route is absent
 * from it. Read the two files together: anything this app serves is either in
 * the sitemap or in this list.
 *
 * `Disallow` is a crawl instruction, not an access control and not a promise of
 * privacy. Everything here is already refused server-side by `expectSession()`,
 * a permission check, or a `CRON_SECRET` bearer token, so this only stops a
 * crawler spending its budget on redirects to `/auth` and on 401s. The pages a
 * signed-in member CAN reach carry `robots: { index: false }` in their own
 * metadata, and that is the half that keeps a URL out of the index if a crawler
 * reaches it by some other path. A line here cannot do that.
 */
export default function robots(): MetadataRoute.Robots {
  return {
    ...
    /**
     * Absolute, and built from the same `BASE_URL` as the sitemap's own `<loc>`
     * entries: a `Sitemap:` line pointing at a different origin than the URLs
     * inside the file is ignored. `.replace` for the same reason as there. The
     * value carries no trailing slash today and this makes that not matter.
     */
