 ...
 *
 * The description is built from the DLW rather than from the request's own
 * `?b=`, because this export cannot see the search params and the club's usual
 * room is the right answer for a bare link. A crawler that follows
 * `?b=Driftmier` gets a slightly generous unfurl; a member who clicks it gets
 * the right map, which is the half that matters.
 */
 ...
 *
 * The building rides in the query string rather than in a route segment, so
 * one page serves all ten and the link a meeting hands out stays pasteable.
...
export default async function DirectionsPage({
  ...
  // exists to reach. A link that names some other building means somewhere
  // else, and gets no room.
  const room =
    typeof r === "string" && r !== ""
      ? r
      : building === "DLW"
