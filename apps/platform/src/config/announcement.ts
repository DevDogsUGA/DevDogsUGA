 ...
 * `ANNOUNCEMENT` and it appears as a card fixed to the bottom of the viewport
 * on every public page. Set `ANNOUNCEMENT` to `null` and it stops rendering
 * entirely: no markup, no script, nothing left in the HTML.
 */
...
export interface Announcement {
   ...
   * whenever it becomes a different announcement. Leave it alone for a typo
   * fix. That is the same announcement, and people who waved it away should
   * not have it waved back at them.
   */
  id: string;
 ...
 * except these: the signed-in console, account, and team pages, where a
 * club-wide notice is noise stacked on top of a working tool. `/oauth` is a
 * third-party consent screen, and nothing of ours belongs in that frame.
 */
 ...
 * the outbound arrow rather than pretending to stay on the site. The
 * homepage's Leadership section carries the same claim behind the same button,
 * so a change to this copy belongs there too. The notice is dismissible and
 * session-scoped; that section is the standing copy that outlives it. See
 * ~/components/LeadershipSection.
 */
 ...
 * attribute.
 *
 * Reading session storage from an effect instead renders the card, paints it,
 * then rips it out a frame later, so anyone who has dismissed the notice
 * watches it flash up from the bottom of every page they load. The markup has
 * to be in the server HTML (it is in the static shell, outside the page's
 * Suspense boundary), so hiding it has to happen before first paint, and only
 * a blocking script runs that early.
 *
 * The root layout renders it rather than AnnouncementBanner, which is where it
 * used to live. A `<script>` returned from a client component only ever
 * executes in the server HTML: React creates a `<div>` in its place on a
