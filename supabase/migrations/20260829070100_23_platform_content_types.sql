-- Content type registry: what an app can have reported, derived from its own
-- schema rather than configured.
--
-- The registration is a foreign key, not a row here. A table becomes moderatable
-- when it carries a foreign key to platform."reportResolutions":
--
--   alter table forum."resources"
--     add column "quarantinedBy" uuid
--       references platform."reportResolutions"(id) on delete set null;
--
-- Nothing is stored on the platform side, so nothing can drift. Drop the column
-- and the table stops being moderatable, with no stale registration left behind
-- to disagree with the schema. That is why there is no registration function, no
-- DDL event trigger, and no webhook back to the app to verify a report. The old
-- rule "an app may only bind tables in its own schema" also stops needing
-- enforcement: the key lives in the app's table, and an app cannot add a column
-- to a schema it does not own.
--
-- The table below is the optional half. A row does one of two jobs: it overrides
-- a derived default (a nicer label, disambiguating two foreign keys to
-- auth.users, trimming a sensitive column out of the snapshot, marking content
-- public), or it declares a type that is reportable but not quarantinable, like
-- the platform's own profile, where the remedy is freezing the name rather than
-- hiding the row. A table with no foreign key to "reportResolutions" is visible
-- to moderation only because a row here says so.
--
-- This file must run after the reports file. content_types() is `language sql`,
-- so its two regclass literals are resolved when the body is parsed at CREATE
-- time, and one of them names "reportResolutions".
...
-- Quarantine does not mean the same thing for every kind of content. For a forum
-- post it means HIDE: the row stops being visible to anyone but its author and a
-- moderator, and the app's own read policy is what does the hiding. For a
-- profile it means FREEZE, because "preferredName" is load-bearing across
-- rosters, teams, attendance and the leaderboard, and a row that vanishes reads
-- as data loss rather than moderation. conformance_report branches on this: told
-- the wrong shape, it reports a correct integration as broken.
...
create table "platform"."contentTypes" (
  ...
  "authorColumn" text,
  -- Identifiers only, verified by the trigger below against pg_attribute and
  -- interpolated with %I when the snapshot is built. A free-text expression here
  -- would be an injection vector authored in a table; expressions belong in a
  -- reviewed "contentResolver" function instead.
  "snapshotColumns" text[],
  -- '{ref}' is replaced with the content reference.
  "urlTemplate" text,
  "visibility"  "platform"."contentVisibility",
  "createdAt"   timestamp without time zone not null default now(),
  "quarantineEffect" "platform"."quarantineEffect",
  constraint "contentTypes_pkey" primary key ("id"),
  constraint "contentTypes_app_table_key" unique ("appId", "tableName"),
  constraint "contentTypes_appId_fkey"
...
-- Two rows naming the same contentType within one app would make resolve_content
-- ambiguous, and contentTypes_app_table_key cannot see the derived default this
-- coalesces to. Only rows that override the default are constrained; the derived
-- defaults are already unique because table names are.
...
-- exactly the category test identities are denied.
...
-- One row per moderatable content type across every registered app. A type
-- appears here when its table carries the foreign key to "reportResolutions", or
-- when a row in "contentTypes" declares it. Everything else is a derived default
-- with an optional override.
--
-- Misconfigured types are returned, not filtered out: "refColumn" and
-- "authorColumn" come back null when they cannot be derived, so
-- conformance_report can say what is wrong. Omitting them would make a broken
-- type indistinguishable from an unregistered one, which is the failure mode
-- this whole design exists to avoid.
--
-- Execute is revoked from clients at the bottom of this file. It exposes the
-- shape of every app's schema, and moderators reach it through the gated
-- list_content_types() instead.
...
--     revoking from anon and authenticated does not touch that grant. They still
--     hold it through PUBLIC.
--   * The schema's `alter default privileges ... grant all on functions to anon,
--     authenticated, service_role` then grants it to them a second time,
--     directly, so revoking from PUBLIC alone is not enough either.
--
