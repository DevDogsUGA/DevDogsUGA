-- A member's Supabase OAuth grant, and the enums the sandbox tables share.
--
-- First of three sandbox migrations. See docs/platform/sandbox-environments.md.
--
-- Nothing here is reachable from a browser. Every column is either a Vault
-- secret id or metadata about somebody's personal Supabase account, and the
-- only legitimate reader is the platform connecting as the owner. The RLS at
-- the bottom is therefore deny-everything rather than the usual
-- public-select-plus-deny-writes shape used elsewhere in this schema.
...
-- is the paid Supabase branching path, which is ruled out today but kept
-- reachable: switching later changes provisioning and nothing else.
...
-- A 'shared' env var has its value inline; a 'secret' one lives in Vault and is
-- never delivered to a member, only used by platform-side operations. That is
-- what keeps the secret key out of .env.local while still living in the same
-- system the members configure.
...
-- whether it works. Storing what was requested at least gives the startup probe
...
-- No permissive policy, so there is nothing to select even before the
-- restrictive ones below. Both halves are stated anyway: RLS with no policy
-- reads as an oversight, and the next person to add a permissive policy for a
-- console page should have to delete an explicit refusal to do it.
