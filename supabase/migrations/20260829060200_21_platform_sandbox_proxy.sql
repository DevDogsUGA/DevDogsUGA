-- Sandbox proxy credentials, the audit trail behind them, and the narrow role
-- the proxy Worker reaches them through.
--
-- Creates platform."sandboxCredentials", platform."proxyRequestLog",
-- platform."envAccessLog", the cluster-level `sandbox_proxy` role, and the two
-- functions that role may call: resolve_sandbox_credential and
-- log_proxy_request.
--
-- The one thing to know: the two `revoke execute` statements at the bottom of
-- each function must keep naming anon, authenticated and service_role. This
-- schema's default privileges grant EXECUTE to those three roles EXPLICITLY,
-- so `revoke ... from public` strips only the PUBLIC entry and leaves a
-- SECURITY DEFINER function that returns decrypted Vault secrets callable by
-- any browser holding an authenticated JWT. Shortening those role lists is the
-- one edit in this file that fails silently.
--
-- This is the only part of the teams work that stays an RPC. Everywhere else
...
--
-- Runs after the sandbox environments file: every FK and the resolve function's
-- body reference platform."sandboxEnvironments".
...
create table "platform"."sandboxCredentials" (
  ...
  -- Opaque random string, stored hashed. The plaintext is shown once at issue
...
-- Server-only, by two independent mechanisms. RLS is on with no permissive
-- policy, so no client role can read a row; the restrictive trio closes the
-- writes that this schema's default privileges opened. The insert, update and
-- delete policies stay split per command because a single restrictive
-- `for all using (false)` would also apply to SELECT, and these names are
-- reused verbatim on other tables, which is legal because policy names are
-- per-table.
...
-- Belt to the policies' braces. RLS decides rows, this decides whether the
-- table is reachable at all, so a policy that is later relaxed by mistake does
-- not by itself expose token hashes over PostgREST.
...
-- The two log tables use a different shape from sandboxCredentials above: one
-- select policy plus one `for all` write policy. `no_client_write` covers
-- SELECT too, so it overlaps `no_client_select` entirely, and both are kept
-- because they are separate rows in pg_policy that tests reference by name.
-- The deny does not depend on either one alone: RLS is on and no permissive
-- policy exists on these tables, so the absence of a read policy is what
-- closes SELECT, and the restrictive policies plus the revokes below close
-- the writes.
--
-- Nothing writes these tables as a client role. log_proxy_request inserts into
-- proxyRequestLog as its definer, which RLS on the table does not restrict
-- because the definer owns it; envAccessLog is written by server actions
-- through Drizzle as the owner.
...
-- second reset fail, hence the guard. Do not simplify it to a bare
-- `create role`.
do $$
begin
...
-- `authenticator` is Supabase's own role, provisioned before any migration.
...
--
--   1. It takes the hostname as well as the token. Resolving the token alone
--      would let a credential minted for environment A be presented at
--      environment B's hostname and resolve successfully, and the Worker would
...
--      `if` in a Worker.
--
--   3. It returns an `outcome` rather than zero rows on failure. The proxy owes
--      an unknown or retired hostname a 410 and a bad credential a 401, and it
--      cannot tell those apart from an empty result. Discriminating here keeps
--      it to one round trip and puts the distinction next to the data that
--      determines it.
--
...
-- PUBLIC still holds EXECUTE on every function in this schema at this point,
-- including the two above, and PUBLIC includes `sandbox_proxy` itself. Closing
-- that is the last migration's job, not this one's: `revoke execute on all
-- functions in schema platform from public` acts only on functions that
-- already exist, so it has to run after the final function is created. The
-- per-role revokes above are what keep these two closed to anon,
-- authenticated and service_role in the meantime.
