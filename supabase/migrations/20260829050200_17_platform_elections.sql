-- Elections and ballots: the rankings teams and officers cast over each
-- other's competition work.
--
-- Creates three enums (electionElectorate, electionPurpose, electionStatus)
-- and three tables: elections, ballots, ballotRankings. Nothing here is an
-- RPC. Casting a ballot and recording a tally are server actions over Drizzle,
-- which connects as the schema owner and bypasses the policies below.
--
-- The one thing to know before reordering anything: the two ballot read
-- policies call platform.has_permission((select auth.uid()), 'canAuditBallots'),
-- and has_permission resolves its second argument as a JSON key over a row of
-- platform."resolvedUserPermissions". An unknown key returns false rather than
-- erroring, so these policies CREATE cleanly even if the canAuditBallots
-- column is missing from roles and from the matview. They just deny every
-- auditor, silently, forever. Run this file after the permission resolution
-- file and after the roles table that carries the column.
...
-- keeps the copy honest: a ballot cannot claim an electorate its election does
-- not have. Same move as "competitionId" on teamMembers.
--
...
create table "platform"."ballots" (
  ...
  -- No delete action given, so this defaults to NO ACTION: deleting an account
  -- that has cast a ballot raises an error rather than removing the vote a
  -- tally was computed from. Adding `on delete cascade` here would let an
  -- account deletion change a finished result.
...
-- ============================================================
--
-- Three tables, twelve policies, and one asymmetry that is the whole design:
-- an election is public and the votes inside it are not. `public_select` on
-- elections reaches anon; the two ballot read policies reach authenticated
-- only and are gated on team membership or canAuditBallots.
--
-- The no_client_* trios are RESTRICTIVE, not permissive, and they are the only
-- thing closing writes: the default privileges in the first migration already
-- granted insert, update and delete on every platform table to anon and
-- authenticated. They are also split per command on purpose. One restrictive
-- `for all ... using (false)` would apply to SELECT too and silently kill the
-- read policies above it.
--
-- Two names repeat here. `own_team_or_auditor_select` is used on both ballots
-- and ballotRankings, and the no_client_* names appear on all three tables and
-- on eleven tables across the teams and elections group. Policy names are
-- scoped per table, so this is legal. A pass that deduplicates by name deletes
-- live policies.
...
-- The same rule as the ballot itself, reached one join away. Rankings carry no
-- team column of their own, so the predicate has to go through ballots. Change
-- the rule above and this copy has to change with it.
