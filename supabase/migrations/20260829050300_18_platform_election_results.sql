...
--
-- Nothing here is client-writable. Every one of these four tables carries the
-- restrictive no_client_insert/_update/_delete trio, so the only writer is the
-- tally running as service_role. What differs table to table is the read rule,
-- and the differences are deliberate: results and standings open only once the
-- election is 'tallied', the pairwise matrix stays behind canAuditBallots
-- forever, and the tiebreak disclosures are public to anon on purpose.
...
--
-- teamA and teamB carry no foreign key, unlike the team columns on
-- tiebreakDisclosures. That asymmetry is intentional: a deleted team must not
-- cascade away the matrix that explains a result already published, whereas a
-- disclosure naming a team that no longer exists says nothing.
...
-- It should produce a visible re-tally instead. Storing the inputs alongside
-- the output is what makes "550 out of 600" reproducible from the row itself.
--
-- totalPoints is generated stored, and competitionStandings_total_in_range is
-- a check over that generated column. Postgres evaluates the generation
-- expression before the check, so the bound applies to the sum rather than to
-- either input, and no writer can set totalPoints directly.
...
-- publishing a running total mid-tally and with it the shape of an unannounced
-- result. With invoker semantics the view sums only rows the reader may
-- already see.
--
...
-- ============================================================
--
-- The restrictive trio is split per command because `for all using (false)`
-- would also kill SELECT. Both halves of each pair matter: the permissive
-- policy is the only thing granting a read, the restrictive ones are the only
-- thing denying a write.
...
-- and in a test rather than encoded here. A rule that depends on nobody adding
-- the obvious insert later is not a rule.
...
-- Officers only, and never opened by a status change: the matrix says how every
-- team ranked every other team, which is the ballot privacy the disclosure
-- table exists to avoid breaking.
...
-- This IS the disclosure surface, so it is public by construction, and the one
-- table in this file whose select policy reaches anon unconditionally.
