-- Who downloaded what.
--
-- `stars.csv` carries member emails, so every download is recorded. This is
-- the protection the design explicitly noted was LOST by exporting attendance
-- from Airtable instead of the platform -- anybody with base access can export
-- an Airtable view silently, and bulk extraction stops being detectable.
--
-- Keeping the one export that survived auditable is what stops that loss from
-- spreading to the export that still holds the most PII.
