-- The sandbox project itself, the teams attached to it, and its env vars.
--
-- Second of three sandbox migrations. Depends on 20260803000002_platform_teams
-- for teamMembers("teamId", "userId", "role"), which teamEnvironments points a
-- composite foreign key at.
...
create table "platform"."sandboxEnvironments" (
  ...
  -- `not null` so there is no third state to reason about -- deleting a user
  -- who owns an environment is refused, and the environment is orphaned through
  -- the documented teardown instead.
  ...
  -- four keys -- anon, service_role, and TWO both literally named "default" --
  -- so matching on the name either picks a deprecated key or is ambiguous.
