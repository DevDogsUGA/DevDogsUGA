...
--
-- Must land before the sandbox work: `teamEnvironments` points a composite
-- foreign key at teamMembers("teamId", "userId", role), so the key backing it
-- has to exist first.
...
-- participation of every team that did not win -- silently, days later, and in
-- a way nobody would connect to the cleanup that caused it.
--
...
create table "platform"."teams" (
  ...
  -- No foreign key, here or on the other audit columns in this migration: the
  -- team outlives the account that created it, and a cascade from auth.users
  -- would delete a competition's history along with a member.
...
-- expiry, the notification, the response audit -- is identical. Two tables
-- would duplicate all of it and then need a union everywhere both are shown.
...
-- checks membership -- Drizzle connects as the owner and is not subject to
-- these grants.
