 ...
 *
 * `resolveEnvironment()` is covered in `load.test.ts`. What is asserted here is
 * the DERIVED half of the table: the subsets and predicates that exist so no
 * command keeps a second copy of "which targets are like this". The second copy
 * this table replaced failed silently. Two enums that agreed on the words
 * `staging` and `production` let `push --env staging` read the development
 * `.env`.
 */
...
describe("holdsOnlyNarrowedKeys", () => {
  it("is exactly the targets that are not deploy environments", () => {
    ...
    // from only feeds CI's plan jobs, and a plan job is narrow by
    // construction", so a future non-booting row inherits the narrowing with
    // no list to remember. A hardcoded `target === "preflight"` would pass a
    // test that only ever named preflight, then hand a new CI-only target
    // every production secret on the day it was added.
  ...
  it("answers for `development` too, which is not a vault target", () => {
    ...
    // to spell the narrow set. It would sweep in the one target whose file is
    // meant to carry every declared key.
