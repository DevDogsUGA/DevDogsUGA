 ...
 * in one manifest, public in another. "Declared in two places" is useless
 ...
 *
 * Called by each app's `env.ts` alongside `createEnv` rather than replacing it.
 * `@t3-oss/env-nextjs` does real work this has no interest in duplicating: the
 * client/server split, the runtime-vs-build distinction, and the proxy that
 ...
 * from every derived artifact, the exact failure the whole package exists to
 ...
 * exclusion, opposite secrecy. Between them they cover every
 ...
 *
 * `minted` is excluded, and the exclusion is fail-closed rather than tidiness:
 * a minted credential is signed at deploy time and has no stored copy, so a
 * value found under that name in somebody's `.env` is a hand-pasted token that
 * must not be propagated. Uploading it would create the long-lived copy the
 ...
 *     the entire point of that gate. Because the ref is a substring of
 ...
 * meaningful on exactly one laptop, a second per-environment home to disagree
 * with.
 *
 ...
 * DEVELOPMENT, so absent from `.env` by design". That is a statement about the
 * local stack and about `.env.example` rendering, not a claim that the value is
 ...
 *
 * Consumed by `env push` (never uploaded) and by `env audit`, where this set
 * is what stops a Worker-only credential being reported, and then pruned, as an
 * orphan. See `EnvMeta.minted`.
 */
 ...
 *
 * Wider than `variableKeys()` because it ignores scope, so it is the wrong set
 * to push FROM. What it is right for is the refusal: nothing here may become a
 ...
 * test is what catches it. Until it does, the safe reading of "one app calls
 * this a secret" is that it is a secret. Requiring every declaration to agree
 * would fail open in exactly the case that matters.
 */
