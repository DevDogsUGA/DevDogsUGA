/**
 * What an `example` MEANS. The one place that decides whether the literal after
 * the `=` is a VALUE or a SHAPE.
 *
 ...
 *
 * Only the first may be carried into a deployed environment. The other two are
 * WRONG there, and because every consumer downstream skips only EMPTY values, a
 * wrong one propagates in silence: pushed to Bitwarden, synced to GitHub, then
 * reported as no-drift by `env audit`, because the stored value does match the
 * file it came from.
 *
 ...
 *     derivation. Shipping it would put a URL with the literal text
