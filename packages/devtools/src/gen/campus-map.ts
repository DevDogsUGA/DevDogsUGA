/**
 * `devtools gen campus-map`
 *
 * Regenerates `apps/platform/src/components/EventsSection/FindUs/campusMapData.ts`
 * (and the few bytes in `campusMapMeta.ts` beside it) from live OpenStreetMap
 * data.
 *
 ...
 * rendered map after regenerating. The command prints every labelled building's
...
function makeGeomHelpers(px: (lon: number, lat: number) => Pt, fmt: (n: number) => number, VH: number) {
  ...
  return { proj, toPath, inFrame, area };
}
...
function clipToFrame(line: Pt[], VH: number, margin = 8): Pt[][] {
...
export async function runGenCampusMap(): Promise<number> {
  const { VH, px, fmt } = makeProjection();
  const { proj, toPath, inFrame, area } = makeGeomHelpers(px, fmt, VH);
 ...
 * their pin.
 */
export const BUILDING_CENTERS: Record<BuildingKey, { lat: number; lon: number }> = ${entries((h) => h.center)};
`;
