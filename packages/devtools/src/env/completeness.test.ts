...
/**
 * The registry's completeness test, the third enforcement layer from the model
 * doc, asserting what the first two cannot.
 *
 ...
 * runtime facts about the whole registry, so they live in a test, and a failure
 * here fails `pnpm test`, not CI and not a deploy.
 */
...
describe("registry completeness", () => {
  ...
  it("duplicate declarations agree on the ENTIRE meta, doc included", () => {
    ...
    // doc is two answers to "what breaks when this is wrong", and whichever one
    // `.env.example` prints, half the readers get the other. The manifest
  ...
  it("uploads no stored credential to the sandbox Worker", () => {
    ...
    // key that mints a token for ANY role, `service_role` included, handed to
    // the one component built to hold EXECUTE on two functions and no table
  ...
  it("⚠️ lets no routed key start with GITHUB_ — GitHub reserves the prefix", () => {
    // Learned the expensive way (2026-08-20): GitHub Actions REFUSES secret and
    // variable names beginning `GITHUB_`, with HTTP 422 "Secret names must not
    // start with GITHUB_". This registry uses ONE name from env file to
    // Bitwarden to GitHub to Worker, so a routed key with the reserved prefix
    // pushes to Bitwarden cleanly and then dies at the GitHub half, leaving the
    // two stores disagreeing. The App credentials renamed to GH_* for exactly
    // this; committed constants (GITHUB_ORG, GITHUB_COMPETITION_REPO) never
    // enter GitHub's stores and keep the honest name.
  ...
  it("pins the narrowed set to the dry run's two credentials", () => {
    ...
    // asserts that what preflight holds under that name cannot do more than a
  ...
  it("keeps `narrowed` on keys that can actually route somewhere", () => {
    ...
    // and it would mean `env init --target preflight` renders no keys.
  ...
  it("never lets a never-store or minted key be narrowed", () => {
    ...
    // put there. Structurally possible today, since `narrowed` is checked
  ...
  it("routes every environment-scoped key to exactly one remote store", () => {
    ...
    // storable, not anything else, and so pushed nowhere. CI never received
    // them, and `pull` on a fresh machine rebuilt a file that could not boot an
    // app.
    //
  ...
  it("keeps the localStack values in the variable set", () => {
    ...
    // DEVELOPMENT, so absent from .env by design". It describes the local stack
    // and how `.env.example` renders. It is NOT a reason to withhold a deployed
    // value, and staging and production have no stack to supply one. Dropping
    // these makes a deploy point at nothing, silently.
    //
  ...
  it("never lets a never-store key become a variable", () => {
    ...
    // the two: `env push` sending BWS_ACCESS_TOKEN to the SECRET store would at
    // least be write-only, whereas the variable store publishes its value to
    // anyone who can read the repository's Actions config.
    const variableSet = new Set(variableKeys());
    for (const key of neverStoreKeys()) {
      expect(
        variableSet.has(key),
  ...
  it("declares every uncommented key in .env.example", async () => {
    ...
    // means invisible to `env push` routing and to drift audits, the original
    // four-files failure surviving in the one file this test can still read.
    // Commented-out lines are prose, not assignments, and are skipped the same
    // way the .env parser skips them.
  ...
  it("renders every declared key into .env.example", async () => {
    // The converse of the check above, made possible now the file is generated:
    // a declared variable that never reaches .env.example is documentation
    // nobody will see. Every key appears as an assignment, commented out
    // (developer scope, disable-by-default), or as a named comment block
    // (never-store), so a plain mention is the right floor.
    const text = await readFile(resolve(PROJECT_ROOT, ".env.example"), "utf8");
