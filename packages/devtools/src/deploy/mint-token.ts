 ...
 * dashboard step, which is what makes rotation cheap enough to do on every
 * deploy rather than a three-step overlap dance. Hence the 90-day `exp`: a
 * pipeline that goes stale then fails loudly instead of quietly holding a
 * credential forever.
 *
 ...
 *      `deploy/report.ts` states, repeated here because this is the file where
 *      breaking it costs the most.
 *   2. **`cli.ts` must dispatch this command BEFORE it calls `intro()`.** That
 *      is a property of the wiring, not of this file, and cannot be defended
 *      from here, so `mint-token.test.ts` asserts it against the text of
 *      `cli.ts`. Wired after `intro()`, this command does not error: it emits
 *      a plausible `┌  DevDogs devtools\n│\n<jwt>` and deploys a malformed
 *      credential that fails later, at the edge, far from here.
 ...
 *      rather than truncated mid-token, and it writes no diagnostic itself:
 *      `cli.ts` renders the thrown `DeployError` through `say()`, on stderr.
 *
 ...
 * above all, should call `mintSandboxToken()` directly instead. It returns the
 * token as a string, which removes the stdout channel, the subprocess, and
 ...
 * `node:crypto` does in one call. A dependency here would expose the one
 * command whose output is a production credential to supply-chain risk, in
 ...
 * exits non-zero rather than printing a stack trace. A caller that uses
 * `mintSandboxToken()` directly (`deploy secrets-file --mint`) gets the same
 * treatment without knowing this subclass exists.
 */
...
export function mintSandboxToken({
  ...
  // and it would produce a well-formed token that anybody can forge.
   ...
   * rather than answering a known requirement. PostgREST does not check either
   * unless configured to, but the hosted API gateway sits in front of it and
   * matching its own tokens costs nothing. `ref` is omitted rather than
