 ...
 * one of its writers — `intro`, `outro`, `log.*`, `note`, the spinner — writes
 * to **stdout**. Verified against @clack/prompts as vendored here on
 * 2026-08-16 by running each of them with the two streams captured
 * separately: stdout got the whole box-drawn transcript and stderr got
 * nothing.
 *
 ...
 * group reports through `say()` — which is stderr — instead of `log`/`note`.
 * A `console.log` anywhere under this directory is a bug: it would put a
 ...
 * grew out of printed by hand: a one-line message, then indented detail
 * naming the thing to go and fix. `cli.ts` renders it to stderr and sets a
 * non-zero exit code; a command that wants the failure in the job summary as
 * well writes that itself, because which failures deserve a summary entry is a
 * per-command judgement (`write-env`'s do; a bad flag does not).
 */
 ...
 *
 * The distinction is what lets `cli.ts` print a message a person can act on
 * instead of a stack trace, and it is worth having here even though both
 * outcomes exit 1: these commands run unattended, and the log line IS the
 * report.
 */
