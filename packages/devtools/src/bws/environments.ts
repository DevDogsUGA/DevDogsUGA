 ...
 *
 * That decision sets the whole budget. Nothing machine-shaped authenticates to
 * Secrets Manager, so there are **no CI machine accounts**, just one `admin`
 * account, held by a person, read/write on all three projects:
 *
 *   | Used | Free plan |
 *   |------|-----------|
 ...
 *
 * The sync costs a second copy that cannot be read back: GitHub secrets are
 * write-only, so `env audit` compares names and `updatedAt` against each
 ...
 * has to be true of a whole target for `pull` to rebuild an env file an app can
 * boot from; while it was true of the secret half only, 27 values existed
...
const SUMMARIES: Record<VaultTarget, string> = {
  ...
  // `env push --target preflight` uploaded all 45 routable keys, including
 ...
 *
 * Assembled from the target table rather than typed out again: a second copy of
 * "staging means staging" is a copy that can disagree, and the disagreement
 * would be silent in exactly the direction that matters.
 */
 ...
 * shape of this mistake was silence: `development` was not in the vault enum,
 * so nothing named it and nothing said why. It is a real target with a real
 * file; what it lacks is a shared credential set to sync against.
 */
 ...
 *
 * Called at the top of every command that talks to a vault, so the refusal is a
 * property of the commands rather than of the argument parser alone: a new
...
// NEVER_SECRET_KEYS, APPLY_ONLY_KEYS) are now DERIVED from the declarations in
// each app's env manifest: `neverStoreKeys()`, `neverSecretKeys()`,
...
// what they could not do is STAY correct, because nothing connected them to the
// schemas they described. A key added to an `env.ts` and not here was pushed to
// Bitwarden as a secret by omission.
//
