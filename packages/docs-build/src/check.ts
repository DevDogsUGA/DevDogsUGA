 ...
 * is the one defect that arrives a paragraph at a time, so nobody is ever the
 * person who broke it.
 *
 * Warn-only is deliberate. A docs lint that fails the build teaches one lesson,
 * how to get under the threshold, and most ways under a word budget are worse
 * than the page that tripped it: detail deleted rather than moved, a paragraph
 * folded into a `<details>` where nobody will look. So this reports and stops
 * there. Nothing here sets an exit code, and `checkDocs` returns rather than
 * throwing, for the same reason the generator does: this module has no business
 * deciding whether a build succeeded.
 *
 ...
 * the code it describes, its collapsibles emitted by `gen/emit.ts` to a fixed
 * shape, and no author chose any of it. Every rule below is about a judgement a
 * person made while writing, so the same rule on a generated page is a warning
 * with nobody to act on it, and several hundred of those would bury the handful
 * that mean something.
 */
...
/**
 * The budgets. They measure nothing: they are the numbers this team agreed are
 * "long enough to be worth a second look", which is all a warn-only threshold
 * has to be.
 */
 ...
 *
 * A line scanner rather than an mdast walk, for three reasons. One rule is
 * about text mdast has already thrown away: a `</summary>` with no blank line
 * under it is a single `html` node, and where the blank lines fall inside it is
 * the whole of what the rule is about. Every warning needs a line number an
 * editor can jump to, which the parsed page cannot give. And this runs on every
 * build, where a second `remark` pass over the whole content tree is a cost the
 * bare mode should not carry.
 *
 * What the scanner owes `parseDocFile` is agreement about what markup is, which
 * is the only reason it tracks code at all, and it has to track all three forms
 * CommonMark has, not the two that are easy. `remark` reads a `#` in a fenced
 * shell transcript as a comment, a `<details>` in a code span as a sentence
 * about collapsibles, and a `<details>` under four spaces of indentation as a
 * sample of the markup rather than the markup itself. The third was checked
 * against `remark` here rather than taken on faith: fed `    <details>` after a
 * blank line, it produces a `code` node holding that text, exactly as it does
 * for a fence.
 *
 ...
 * count down to whatever sat above the tag. The rule that matters most on a
 * long page then goes quiet on the longest pages of all.
 */
export function checkDocFile(source: string, file: string): CheckWarning[] {
  ...
  for (let index = bodyStart(lines); index < lines.length; index += 1) {
    ...
    // A closing tag with nothing open is counted twice: once in the raw total
    // the warning quotes, and once as a crossed pair. Without the second number
    // a page that closes a collapsible it never opened and then opens one it
    // never closes balances on paper, one and one, and reports nothing at all,
    ...
    // own line is the same defect as text on the line below it. Checked against
    // `parseDocFile` rather than assumed: both forms come back with
 ...
 * every page that has one, the difference between a warning you can click and a
 * warning you have to go hunting for.
 *
 * That the delimiter has to sit at the margin is the one place this can
 * disagree with `matter()` and quietly skew every count on the page. A
 * `description: |` opens a YAML block scalar, and a `---` indented inside one
 * is a line of the description. gray-matter reads it that way: put to it here,
 * it returns the whole scalar and a body that starts after the real delimiter.
 * So a scanner that stopped at the indented line would count the remaining YAML
 * as body words toward `page-length` and `missing-description`. The opening
 * delimiter runs the other way: gray-matter takes a file that does not begin
 * with `---` as having no front matter at all, so a ` ---` first line is body,
 * and stepping over it would hide real lines from every rule.
 */
 ...
 *
 * A code span cannot open an element. `` `<details>` `` on a page is a sentence
 * about a collapsible, not a collapsible, and this repository writes that
 * sentence: the comments above say it, and `gen` prints these very doc comments
 * onto a reference page. Matching a run of backticks against a closing run of
 * the same length is CommonMark's own rule, which is what makes a span holding
 * a backtick work.
 */
