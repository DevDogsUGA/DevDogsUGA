 ...
 * enough that two copies would drift the first time either was corrected. So
 ...
 * a ```` ```ts ````, but forbids a backtick anywhere inside it. So a line whose
 * leading backtick run is followed by another backtick on the same line opens
 * nothing at all. It stays an ordinary paragraph, and its backtick runs pair
 * into code spans the way any other paragraph's do. Rendered through the site's
 * own plugins, `remark-gfm` into `rehype-raw`, a line reading
 ...
 * paragraph into a space, so a fenced sample written across four lines reaches
 * the emitter as one line. Read as an opening fence, it is a fence nothing
 * closes, and every `<` in the rest of the symbol's prose is handed to the page
 * untouched. A `<details>` in a sentence below it then opens a disclosure
 * element that is still open when the page ends, so the emitted markdown
 * renders the closing `</details>` after the source link with the whole
 * remainder of the symbol inside it. That is the failure the escaping in
 * `gen/emit.ts` exists to prevent, arriving through a door escaping never sees.
 *
 ...
 * runs to the end of the text, rendering everything after it inside a single
 * `<pre><code class="language-dart">`, so that line still opens a fence here:
 * a fence is what the reader gets.
 */
 ...
 * writing-docs.md shows how to write a code block (its "Code blocks" section
 * wraps a ```` ```typescript ```` example in a four-backtick fence).
 */
