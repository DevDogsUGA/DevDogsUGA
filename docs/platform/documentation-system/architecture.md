...
`pnpm docs:index` (`devtools docs index`) reads the same build-time artifact, upserts every page by `path`, and deletes rows whose path no longer exists — one transaction. The `search` column is `generated always`, so the script writes only title/description/plainText and Postgres recomputes the vector.
